...
agv/{agvCode}/command             # 控制指令 (暂停/继续)
```
...
- 小车端建议对关键消息（任务进度、异常）使用 QoS=1，状态消息可使用 QoS=0 以减少网络开销
...
| batteryVoltage | double | ✓ | 电池电压真实值 (V)，服务器端会根据电压计算电量百分比 |
| speed | double | ✓ | 当前速度 (m/s) |
| position | object | ✓ | 位置信息 |
| position.x | double? | - | X坐标 (厘米) |
| position.y | double? | - | Y坐标 (厘米) |
| position.angle | double? | - | 朝向角度 (0-360度，正东为0) |
| position.stationCode | string? | - | 当前站点编码，不在站点时为null |
...
| progressPercentage | double? | - | 进度百分比 (0-100) |
...
| position | object? | - | 发生异常时的位置信息 |
| position.x | double? | - | X坐标 (厘米) |
| position.y | double? | - | Y坐标 (厘米) |
| position.angle | double? | - | 朝向角度 (0-360度，正东为0) |
| position.stationCode | string? | - | 当前站点编码 |
...
public class MqttBrokerService : IHostedService, IMqttBrokerService, IDisposable
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        _mqttServer.ValidatingConnectionAsync += async e =>
        {
            ...
            // 验证密码（返回元组：(bool isValid, string message)）
...
4. **连接认证**：验证ClientId必须等于Username，且都是小车编码（如V001），密码通过数据库验证
5. **消息持久化**：所有入站和出站消息都异步持久化到数据库MqttMessageLog表，不阻塞消息处理流程
6. **在线状态管理**：客户端连接/断开时自动更新数据库中的小车状态和最后在线时间
7. **QoS策略**：服务器端发送的所有消息统一使用QoS=1（AtLeastOnce），Retain=false
...
| v1.8 | 2026-01-22 | - | **根据实际代码更新 8.1 MQTT Broker 实现文档**：①修正密码验证方法返回类型为元组 `(bool isValid, string message)`；②优化关键设计要点为表格展示；③新增 8.1.1 MqttMessageHandler 详细实现说明（状态上报、任务进度、异常处理）；④新增 8.1.2 MqttTopics 常量定义说明；⑤新增 8.1.3 实现清单和依赖关系；⑥补充实际代码的职责分工和设计细节 |
| v1.9 | 2026-01-22 | - | **优化文档结构和准确性**：①删除8.1节中的实现细节（8.1.1、8.1.2、8.1.3），只保留架构设计要点；②删除未实现的路径锁定相关Topics（path/lock-request、path/lock-response、path/unlock）及其消息定义（5.2.7、5.2.8、5.2.9）；③为所有消息字段添加必填/选填标注（✓/-）；④更新状态上报消息字段 battery 为 batteryVoltage（电压值）；⑤统一位置信息字段名 stationId 为 stationCode |
