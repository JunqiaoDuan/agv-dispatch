...
/// 等待任务创建对话框ViewModel
/// </summary>
public partial class ReturnToWaitingDialogViewModel : ObservableObject
{
    ...
    public ReturnToWaitingDialogViewModel(IAgvApiService agvApiService)
    {
    ...
    {
        // 筛选等待区站点
        var standbyStations = stations.Where(s => s.StationType == StationType.Standby).ToList();
        Stations = new ObservableCollection<StationListItemDto>(standbyStations);
    ...
    private async Task LoadPendingItemsAsync()
    {
        ...
        try
        {
            var items = await _agvApiService.GetPendingReturnAgvsAsync();
